...
void init_mysql_conf() {
	...
	if (fp != NULL) {
		while (fgets(buf, BUFFER_SIZE - 1, fp)) {
            ...
            //对redis的支持
            read_int(buf, "OJ_CLI_REDISENABLE", &oj_redis);
            read_buf(buf, "OJ_CLI_REDISSERVER", oj_redisserver);
...
void update_solution(int solution_id, int result, int time, int memory, int sim,
	...
	if (http_judge) {
        ...
        if(oj_redis == 1){
            _update_solution_redis(solution_id, result, time, memory, sim, sim_s_id,
    				pass_rate);
        }else{
...
void addreinfo(int solution_id) {
	if (http_judge) {
        ...
        if(oj_redis){
            _addreinfo_redis(solution_id, "error.out");
        }else{
...
void adddiffinfo(int solution_id) {
	...
	if (http_judge) {
        ...
        if(oj_redis){
            _addreinfo_redis(solution_id, "diff.out");
        }else{
...
void addcustomout(int solution_id) {
	...
	if (http_judge) {
        ...
        if(oj_redis){
            _addreinfo_redis(solution_id, "user.out");
        }else{
...
int init_mysql_conn() {
    do{
        if(oj_redis){
            oj_rediscli = redisConnect(oj_redisserver, oj_redisport);
            break;
        }
...
void get_solution(int solution_id, char * work_dir, int lang) {
	if (http_judge) {
        ...
        if(oj_redis){
            _get_solution_redis(solution_id, work_dir, lang);
        }else{
...
void get_custominput(int solution_id, char * work_dir) {
	if (http_judge) {
        ...
        if(oj_redis){
            _get_custominput_redis(solution_id, work_dir);
        }else{
...
void get_solution_info(int solution_id, int & p_id, char * user_id,
	...
	if (http_judge) {
        ...
        if(oj_redis == 1){
            _get_solution_info_redis(solution_id, p_id, user_id, lang);
        }else{
...
int main(int argc, char** argv) {
	...
	int Compile_OK;
	//为了让此判题系统支持考试系统的多文件include，所以将需要的文件都复制到工作目录下
	...
	//为了让此判题系统支持考试系统的文件题，所以将需要的文件都复制到工作目录下
        execute_cmd("/bin/cp %s/data/%d/* %s/", oj_home, p_id, work_dir);
        //end
